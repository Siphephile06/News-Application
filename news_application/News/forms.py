...
class ArticleForm(forms.ModelForm):
    ...
    class Meta:
        model = Article
        fields = ['headline', 'byline', 'body', 'conclusion', 'approved']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and hasattr(user, 'profile') and user.profile.is_verified_contributor:
            ...
            self.fields['publisher'].queryset = Publisher.objects.filter(name="Independent")
...
class PublisherForm(forms.ModelForm):
    ...
    editor = forms.ModelMultipleChoiceField(
        ...
        widget=forms.SelectMultiple(attrs={'class': 'form-control', 'placeholder':
                                   'Editor'})
    )
